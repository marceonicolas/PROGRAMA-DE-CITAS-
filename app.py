...
# --- CODIGO DE EMERGENCIA: BORRAR DESPUÉS DE ENTRAR ---
def crear_usuario_inicial():
    usuario_test = "admin_alborada"
    clave_test = "admin123"
    
    # Verificamos si ya existe para no duplicar
    existe = supabase.table("usuarios").select("*").eq("usuario", usuario_test).execute()
    
    if not existe.data:
        h_pass = hash_password(clave_test)
        supabase.table("usuarios").insert({
            "usuario": usuario_test, 
            "password": h_pass, 
            "rol": "admin"
        }).execute()
        st.success("✅ Usuario inicial creado desde Python. ¡Ya puedes entrar!")

# Llama a la función justo aquí
