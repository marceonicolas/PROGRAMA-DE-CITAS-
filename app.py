                ...
                with st.expander(f"{emoji} {row['nombre']} {row['apellido']} - {row['fecha_cita']} {row['hora']}"):
                    ...
                    with col_acc1:
                        ...
                        if row['estado'] == 'no asistio':
                            st.write("**Seguimiento de 4 Semanas:**")
                            s_cols = st.columns(4)
                            for i in range(1, 5):
                    ...
                    with col_acc2:
                        ...
                        if nuevo_estado == "reagenda":
                            ...
                            if st.button("Confirmar Reagendamiento", key=f"br_{row['id']}", use_container_width=True):
                                supabase.table("pacientes").update({
                                    "estado": "pendiente", 
                                    "fecha_cita": str(n_fecha), 
                                    "hora": str(n_hora)
                                }).eq("id", row['id']).execute()
                                st.rerun()
                        
                        elif st.button("Guardar Estado", key=f"sv_{row['id']}", use_container_width=True):
