                ...
                with st.expander(f"📅 {fecha_formato} | 🕒 {row['hora']} - {row['nombre']} {row['apellido']} ({row['estado']})"):
                    ...
                    with col2:
                        ...
                        if row['estado'] == "No Asistió":
                            msg_reag = f"Hola {nombre_full}, lamentamos que no hayas podido asistir a tu cita el {fecha_formato}. ¿Te gustaría reagendar para otro día?"
                            st.link_button("🔄 Invitar a Reagendar", crear_link_wa(row['telefono'], msg_reag), use_container_width=True)
                        
                        msg_sem = f"Hola {nombre_full}, te saludamos de Alborada para saber si todo va bien con tu proceso. ¡Estamos a las órdenes!"
