    ...
    if choice == "Agenda":
        ...
        if not df.empty:
            for i, row in df.iterrows():
                with st.expander(f"🕒 {row['hora']} - {row['nombre']} {row['apellido']} ({row['creado_por']})"):
                    col1, col2 = st.columns(2)
                    with col1:
                        nuevo_est = st.selectbox("Estado", ["Pendiente", "Asistió", "No Asistió", "Reagendado", "Cliente firmó"], 
                        ...
                        if st.button("Guardar", key=f"btn_{row['id']}"):
                            ...
                            st.rerun()
                    with col2:
                        # Solo el Admin o el creador pueden borrar registros individuales
