...
def login():
    ...
    if st.button("Ingresar"):
        ...
        if res.data:
            ...
            # --- SUPER BYPASS TEMPORAL ---
            # Si la clave coincide exactamente (texto plano) O coincide el hash
            es_valida = False
...
if st.session_state.logged_in:
    ...
    # 1. Definimos las opciones del menú en la barra lateral
    st.sidebar.title(f"Hola, {user['usuario']}")
    st.sidebar.write(f"Rol: {user['rol'].capitalize()}")
    
    opciones = ["Mis Agendamientos", "Registrar Paciente", "Reporte Diario"]
    
    # Solo el admin ve el panel para crear otros usuarios
    if user['rol'] == 'admin':
        opciones.append("Panel Supervisor")
        
    choice = st.sidebar.selectbox("Ir a:", opciones)
    st.sidebar.divider()
        ...
        with st.form("form_registro", clear_on_submit=True):
            ...
            if btn_guardar:
                if nombre and apellido and telefono:
                    ...
                    st.warning("Nombre, Apellido y Teléfono son obligatorios.")

    # --- SECCIÓN: MIS AGENDAMIENTOS ---
    elif choice == "Mis Agendamientos":
        st.header("📅 Mis Gestiones")
        st.info("Aquí solo ves los pacientes que tú registraste.")
        
        # Filtro de fecha para el asesor
        f_inicio = st.date_input("Desde", datetime.now())
        
        # Consultamos solo lo que pertenece al ID del usuario actual
        data = supabase.table("pacientes").select("*")\
        ...
        if data:
            for row in data:
                with st.expander(f"{row['hora']} - {row['nombre']} {row['apellido']} ({row['estado']})"):
                    st.write(f"**CI:** {row['ci']} | **Tel:** {row['telefono']}")
                    st.write(f"**Notas:** {row['observaciones']}")
                    # Aquí podrías poner tus botones de WhatsApp
        else:
            st.write("No tienes agendamientos para esta fecha.")

    # --- SECCIÓN: REPORTE DIARIO ---
    elif choice == "Reporte Diario":
        # Aquí irá el código de la tabla general y descarga de Excel que vimos antes
        st.header("📊 Reporte para la Clínica")
