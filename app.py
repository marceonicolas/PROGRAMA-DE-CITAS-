...
if not st.session_state['logged_in']:
    ...
    if choice == "Agenda":
        ...
        with col_mes:
            if st.button("📅 Seleccionar Mes Actual"):
                ...
                # Forzamos el rango en el widget (se aplicará al recargar)
                st.session_state['rango_manual'] = [inicio_mes.date(), fin_mes.date()]
                st.rerun()

        # Lógica de filtrado SQL basada en el rango
        if isinstance(rango_fechas, list) or isinstance(rango_fechas, tuple):
            if len(rango_fechas) == 2:
                ...
                # Caso donde solo se ha hecho clic en una fecha (inicio)
                inicio = rango_fechas[0]
                query = f"SELECT * FROM clientes WHERE fecha = '{inicio}'"
        
        # Filtro de seguridad por rol (Administrador ve todo, Asesor solo lo suyo)
        if st.session_state['rol'] != "Administrador":
        ...
        if not df.empty:
            ...
            for i, row in df.iterrows():
                # Formateamos la fecha para mostrarla en el expander
   ...
   # 2. REGISTRAR CLIENTE
    # 2. REGISTRAR CLIENTE
        ...
        # Inicializamos las claves en session_state si no existen
        for key in ['n_input', 'a_input', 'ci_input', 'tel_input']:
            if key not in st.session_state:
                st.session_state[key] = ""

        # Fila 1: Nombre y Apellido
        row1_col1, row1_col2 = st.columns(2)
        n = row1_col1.text_input("Nombre", key="n_input")
        a = row1_col2.text_input("Apellido", key="a_input")
        
        # Fila 2: CI y Teléfono
        row2_col1, row2_col2 = st.columns(2)
        ci = row2_col1.text_input("CI", key="ci_input")
        tel = row2_col2.text_input("Teléfono", key="tel_input")
        ...
        if st.button("Guardar Registro", type="primary"):
            if n and a: # Validación básica
                ...
                # --- LIMPIEZA AUTOMÁTICA ---
                # Borramos el contenido de los inputs en el session_state
                st.session_state['n_input'] = ""
                st.session_state['a_input'] = ""
                st.session_state['ci_input'] = ""
                st.session_state['tel_input'] = ""
                
                st.success(f"¡Cliente {n} {a} guardado con éxito! Los campos se han limpiado.")
                st.rerun() # Recarga para mostrar los campos vacíos inmediatamente
            else:
