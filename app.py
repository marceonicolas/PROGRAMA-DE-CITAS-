                ...
                with st.expander(f"📅 {fecha_formato} | 🕒 {row['hora']} - {row['nombre']} {row['apellido']} ({row['estado']})"):
                    ...
                    with col2:
                        ...
                        if row['estado'] == "No Asistió":
                            msg_reag = f"Hola {nombre_full}, lamentamos que no hayas podido asistir a tu cita el {fecha_formato}. ¿Para que fecha te gustaria reagendar?"
                            st.link_button("🔄 Invitar a Reagendar", crear_link_wa(row['telefono'], msg_reag), use_container_width=True)
                        
                        msg_sem = f"Hola {nombre_full}, ¿cómo estás? Paso a saludarte y a recordarte que seguimos con el espacio disponible para tu consulta de [Tratamiento].

Cuidar tu salud dental/estética a tiempo siempre es la mejor inversión. ¡Quedo atento por si lograste revisar tus horarios para esta semana! Saludos."
                        st.link_button("💬 Seguimiento Semanal", crear_link_wa(row['telefono'], msg_sem), use_container_width=True)
