...
try:
    # Intentamos leer solo los nombres de usuario para ver si hay conexión
    test_res = supabase.table("usuarios").select("username").execute()
    st.info(f"Conexión exitosa. Usuarios en la nube: {[u['username'] for u in test_res.data]}")
except Exception as e:
    st.error(f"Error de conexión: {e}")
    
    if st.button("Entrar"):
        hashed: str = hash_pass(pwd)
